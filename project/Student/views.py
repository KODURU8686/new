...
def tab(request):
	...
	alldonors=MedicineInfo.objects.filter(accepted_by="None")
	...
	f = k.values()
	return render(request,'htfiles/table.html',{'info':f,'donorlist':alldonors,'variable':value,'variable1':email})
...
def accept(request):
    id1=request.POST.get('accepting')
    naam=request.POST.get('ngokanaam')
    print(naam)
    email=request.POST.get('email')
    print(email)
    MedicineInfo.objects.filter(uid_id=id1).update(accepted_by=naam)
    r=MedicineInfo.objects.get(uid_id=id1)
    return render(request, 'table.html',{"variable":naam,"variable1":email}) 
    
def donorhistory(request):
	value=request.POST.get("donorkaemail")
	h=MedicineInfo.objects.filter(email=value)
	print(h)
	return render(request,'donorhistory.html',{'info':h})
    
def ngohistory(request):
    value=request.POST.get("ngokanaam")
    j=MedicineInfo.objects.filter(accepted_by=value)
    print(alldata)
    return render(request,'ngohistory.html',{'info':j})	